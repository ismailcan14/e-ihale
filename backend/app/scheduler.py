...
def activate_auctions():
    ...
    now = datetime.utcnow()
    auctions = db.query(Auction).filter(Auction.start_time <= now, Auction.is_active == False).all()
...
def deactivate_auctions_and_select_winner():
    db: Session = SessionLocal()
    now = datetime.utcnow()
    auctions = db.query(Auction).filter(Auction.end_time <= now, Auction.is_active == True).all()
