...
export default function FinishedAuctionsPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://127.0.0.1:8000/auctions/my", {
      ...
      .then((res) => res.json())
      .then((data) => {
        const filtered = data.filter((a: any) => a.status === "finished");
        setFinishedAuctions(filtered);
      })
      .catch((err) => console.error("Bitmiş ihaleler alınamadı:", err));
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 py-10 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-lg text-center text-gray-600 dark:text-gray-300 mb-10 max-w-2xl mx-auto">
          Süresi dolmuş tüm ihalelerinizi ve sonuçlarını buradan görüntüleyebilirsiniz.
        </p>
